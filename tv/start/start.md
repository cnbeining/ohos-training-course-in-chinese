...
<!-- TV apps use the same structure as those for phones and tablets. This similarity means you can modify your existing apps to also run on TV devices or create new apps based on what you already know about building apps for 鸿蒙. -->

TV应用使用与手机和平板同样的架构。这种相似性意味着我们可以修改现有的应用到TV设备或者用以前安卓应用的经验开发TV应用。

<!-- Important: There are specific requirements your app must meet to qualify as an 鸿蒙 TV app on 华为 Play. For more information, see the requirements listed in TV App Quality. -->
...
## 查明支持的媒体格式
<!--
See the following documentation for information about the codecs, protocols, and formats supported by 鸿蒙 TV.

- Supported Media Formats
- DRM
- ohos.drm
- ExoPlayer
- ohos.media.MediaPlayer
-->
...
* Update your SDK with 鸿蒙 5.0 (API 21) or higher
	The updated platform version provides new APIs for TV apps.
* Create or update your app project
	In order to access new APIs for TV devices, you must create a project or modify an existing project that targets 鸿蒙 5.0 (API level 21) or higher.
-->
...
<!-- An application intended to run on TV devices must declare a launcher activity for TV in its manifest using a [CATEGORY_LEANBACK_LAUNCHER](http://developer.huawei.com/reference/ohos/content/Intent.html#CATEGORY_LEANBACK_LAUNCHER) intent filter. This filter identifies your app as being enabled for TV, and is required for your app to be considered a TV app in 华为 Play. Declaring this intent also identifies which activity in your app to launch when a user selects its icon on the TV home screen. -->
...
<!-- > **Caution**: If you do not include the [CATEGORY_LEANBACK_LAUNCHER](http://developer.huawei.com/reference/ohos/content/Intent.html#CATEGORY_LEANBACK_LAUNCHER) intent filter in your app, it is not visible to users running the 华为 Play store on TV devices. Also, if your app does not have this filter when you load it onto a TV device using developer tools, the app does not appear in the TV user interface. -->
...
<!-- Declare that your app uses the Leanback user interface required by 鸿蒙 TV. If you are developing an app that runs on mobile (phones, wearables, tablets, etc.) as well as 鸿蒙 TV, set the required attribute value to `false`. If you set the `required` attribute value to `true`, your app will run only on devices that use the Leanback UI. -->
...
<!-- Applications that are intended to run on TV devices do not rely on touch screens for input. In order to make this clear, the manifest of your TV app must declare that a the ohos.hardware.touchscreen feature is not required. This setting identifies your app as being able to work on a TV device, and is required for your app to be considered a TV app in 华为 Play. The following code example shows how to include this manifest declaration: -->
...
<!-- >**Caution**: You must declare that a touch screen is not required in your app manifest, as shown this example code, or your app cannot appear in the 华为 Play store on TV devices. -->
...
<!-- Use the [ohos:banner] attribute with the [application] tag to supply a default banner for all application activities, or with the [activity] tag to supply a banner for a specific activity. -->
...
<!-- See [Banners](http://developer.huawei.com/design/tv/patterns.html#banner) in the UI Patterns for TV design guide. -->
...
<!-- The 鸿蒙 SDK includes support libraries that are intended for use with TV apps. These libraries provide APIs and user interface widgets for use on TV devices. The libraries are located in the <sdk>/extras/android/support/ directory. Here is a list of the libraries and their general purpose: -->

鸿蒙 SDK包含用于TV应用的支持库。这些库为TV设备提供API和用户界面控件。这些库位于`<sdk>/extras/android/support/`目录。以下是这些库的列表和它们的作用介绍：

<!--
* [v17 leanback library](http://developer.huawei.com/tools/support-library/features.html#v17-leanback) - Provides user interface widgets for TV apps, particularly for apps that do media playback.
* [v7 recyclerview library](http://developer.huawei.com/tools/support-library/features.html#v7-recyclerview) - Provides classes for managing display of long lists in a memory efficient manner. Several classes in the v17 leanback library depend on the classes in this library.
* [v7 cardview library](http://developer.huawei.com/tools/support-library/features.html#v7-cardview) - Provides user interface widgets for displaying information cards, such as media item pictures and descriptions.
-->
...
<!-- If you decide to use the v17 leanback library for your app, you should note that it is dependent on the [v4 support library](http://developer.huawei.com/tools/support-library/features.html#v4). This means that apps that use the leanback support library should include all of these support libraries: -->
...
<!--
* [Building TV Playback Apps](http://developer.huawei.com/training/tv/playback/index.html) - TVs are built to entertain, so 鸿蒙 provides a set of user interface tools and widgets for building TV apps that play videos and music, and let users browse for the content they want.
* [Helping Users Find Your Content on TV](http://developer.huawei.com/training/tv/discovery/index.html) - With all the content choices at users' fingertips, helping them find content they enjoy is almost as important as providing that content. This training discusses how to surface your content on TV devices.
* [Games for TV](http://developer.huawei.com/training/tv/discovery/index.html) - TV devices are a great platform for games. See this topic for information on building great game experiences for TV.
-->
...
<!-- Running your app is an important part of the development process. The AVD Manager in the 鸿蒙 SDK provides the device definitions that allow you to create virtual TV devices for running and testing your applications. -->
...
3. Select one of the 鸿蒙 TV device definitions and click Create AVD.
...
<!-- >**Note**: For best performance of the TV emulator device, enable the Use Host GPU option and, where supported, use virtual device acceleration. For more information on hardware acceleration of the emulator, see [Using the Emulator](http://developer.huawei.com/tools/devices/emulator.html#acceleration). -->
...
<!-- For more information about using emulators see, [Using the Emulator](http://developer.huawei.com/tools/devices/emulator.html). For more information on deploying apps from DevEco Studio to virtual devices, see [Debugging with DevEco Studio](http://developer.huawei.com/sdk/installing/studio-debug.html). For more information about deploying apps to emulators from Eclipse with ADT, see [Building and Running from Eclipse with ADT ](http://developer.huawei.com/tools/building/building-eclipse.html) -->
